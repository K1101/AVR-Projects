...
; 0000 0085 TWCR=(0<<TWEA) | (0<<TWSTA) | (0<<TWSTO) | (0<<TWEN) | (0<<TWIE);
	OUT  0x36,R30
; 0000 0086 
; 0000 0087 DDRD=0xF8; //  three low value bit are input
	LDI  R30,LOW(248)
	OUT  0x11,R30
; 0000 0088 DDRA=0xFF;
	LDI  R30,LOW(255)
...
_0x3:
; 0000 008C 
; 0000 008D         i = PIND & 0b00000111;
	IN   R30,0x10
	ANDI R30,LOW(0x7)
	MOV  R18,R30
	CLR  R19
; 0000 008E         PORTC=(0b00000001 << i);
	LDI  R26,LOW(1)
	CALL __LSLB12
	OUT  0x15,R30
; 0000 008F         //part 2
; 0000 0090           //in port A => 0001 --> 0010 --> 0100 --> shift(1000) then move bit no.3 to bit no.0 --> 0001 --> 0010 , . . . ...
; 0000 0091         temp = PORTA;
	IN   R16,27
; 0000 0092         temp = temp<<1;
	LSL  R16
; 0000 0093         if ((temp & 0x08)!=0 ){
	SBRS R16,3
	RJMP _0x6
; 0000 0094             temp = ~(temp) & 0x01 ;
	MOV  R30,R16
	COM  R30
	ANDI R30,LOW(0x1)
	MOV  R16,R30
; 0000 0095         }
; 0000 0096         PORTA = temp;
_0x6:
	OUT  0x1B,R16
